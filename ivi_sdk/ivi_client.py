...
class IVIClient:
    """
    A simple, thin wrapper and management layer for the IVI protobuf & gRPC API.
    ...
    This class supports `async with` semantics, otherwise remember to call close() 
    when done.
    ...
    class IVIMetadataPlugin(grpc.AuthMetadataPlugin):
        """
        Internal adapter class for sending the api key as gRPC metadata to the server.
        """
    ...
    def __init__(self, host:str, envid:str, apikey:str,
        ...
        ----------
        host : str
            Server endpoint.  The production server is given by the class attribute default_host
        envid : str
        ...
        on_item_status_update : callable(ItemStatusUpdate)
            Called when an ItemStatusUpdate from ivi_sdk.streams.item.stream_pb2 is received
        on_itemtype_status_update : callable(ItemTypeStatusUpdate)
            Called when an ItemTypeStatusUpdate from ivi_sdk.streams.itemtype.stream_pb2 is received
        on_order_status_update : callable(OrderStatusUpdate)
            Called when an OrderStatusUpdate from ivi_sdk.streams.order.stream_pb2 is received
        on_player_status_update : callable(PlayerStatusUpdate)
            Called when a PlayerStatusUpdate from ivi_sdk.streams.player.stream_pb2 is received
        channel_options : List of tuples
            Custom gRPC options
        ...
        if channel is None:
            ...
            metadata_credentials = grpc.metadata_call_credentials(IVIClient.IVIMetadataPlugin(apikey))
            self.channel = grpc.aio.secure_channel(
                target=host,
                credentials=grpc.composite_channel_credentials(ssl_credentials, metadata_credentials),
        ...
        self._itemtype_status_stream_it = self._itemtype_stream.ItemTypeStatusStream(subscribe_request)
        self._order_stream = OrderStreamStub(self.channel)
        self._order_status_stream_it = self._order_stream.OrderStatusStream(subscribe_request)
