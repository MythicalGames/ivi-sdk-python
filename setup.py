...
class ivi_build_py(build_py):
    def run(self):
        ...
        subprocess.check_call(['git', 'clone', '-b', 'v2.0', '--single-branch', '--depth', '1', 'https://github.com/MythicalGames/ivi-sdk-proto', 'ivi-sdk-proto'])
        protopaths = list(map(str, Path('ivi-sdk-proto').rglob('*.proto')))
        proto_include = pkg_resources.resource_filename('grpc_tools', '_proto')
        if 0 != protoc_main(['grpc_tools.protoc', '-I{}'.format(proto_include), '-Iivi-sdk-proto', '--python_out=ivi_sdk', '--grpc_python_out=ivi_sdk'] + protopaths):
            raise RuntimeError("protoc failed, see previous")
        # hackaround: protobuf still doesn't correctly specify imports for Python3
