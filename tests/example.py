#!/usr/bin/env python3
"""
Example script for registering callbacks and calling API functions through the ivi_sdk ivi_client.
Usage:
...
async def run(ivi_client:IVIClient, sleep_for:int=60, max_loop:int=2):
    ...
    print('Will issue', max_loop, 'items, sleeping between each issue for', sleep_for, 'seconds, then terminating for issuing one too many items')

    try:

        # These examples await results of RPC calls, which you may or may not want
        # to do depending on your program's requirements and threading structure.
        # In most cases, you will likely instead want to wrap each RPC in an async 
        # function which includes your own logic for processing the result, and add 
        # it to your preferred event loop.

        import uuid
        from google.protobuf.struct_pb2 import Struct, Value
        from ivi_sdk.common.common_pb2 import Metadata as Metadata
        ...
        from ivi_sdk.api.order.definition_pb2 import CreateOrderRequest, Address, OrderRequestProviderProto, BitPayOrderRequestProto, ItemTypeOrders, ItemTypeOrder
        from ivi_sdk.api.order.payment_pb2 import PaymentProviderId
        from ivi_sdk.api.payment.definition_pb2 import CreateTokenRequest
        from ivi_sdk.api.player.definition_pb2 import GetPlayerRequest, LinkPlayerRequest
        ...
        create_itemtype_response = await ivi_client.itemtype_service.CreateItemType(create_itemtype_request)
        print('Item Type created:')
        print(create_itemtype_response)

        # now just loop making players and issuing items till we hit max_supply hard coded above, which raises the below-caught exception
        while True:
            ...
            link_player_response = await ivi_client.player_service.LinkPlayer(link_player_request)
            print('Player linked:')
            ...
            issue_item_response = await ivi_client.item_service.IssueItem(issue_item_request)
            print('Item issued:')
        ...
        if issued_item_count == max_loop and e.code() == grpc.StatusCode.FAILED_PRECONDITION:
            print('\nExample IssueItem loop complete, here is the expected Exception from issuing one too many of an ItemType:\n\n', e)
        else:
...
async def main(ivi_server:str = None, ivi_envid:str = None, ivi_apikey:str = None):
    """
    ...
    if ivi_server == IVIClient.default_host:
        raise RuntimeError('Please do not run the example program against the production server')

    # global exception handler
    asyncio.get_event_loop().set_exception_handler(handle_exception)

    # initialize the ivi_client, we can use async with or explicitly call ivi_client.close() when done
    async with IVIClient(ivi_server, ivi_envid, ivi_apikey,
                        item_update, itemtype_update, order_update, player_updated) as ivi_client:

        # required - stream updates need confirmation messages sent back to the server
        # exact scheduling semantics are left up to sdk users
        subcoros = [asyncio.ensure_future(coro()) for coro in ivi_client.coroutines()]

        # kick off our example API calls which will push stream updates back to us
